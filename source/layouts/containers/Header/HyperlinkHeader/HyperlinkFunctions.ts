...
export function loadSoftware(blockAction: 'overtime' | 'ticketing' | 'hyperlink') {
  const hyperlinkBody = document.getElementById('hyperlink-body') as HTMLDivElement;
  const activateBody = document.getElementById(`${blockAction}-body`) as HTMLDivElement;

  if (hyperlinkBody.classList.contains('active')) {
    hyperlinkBody.classList.add('asleep');
    hyperlinkBody.classList.remove('active');
