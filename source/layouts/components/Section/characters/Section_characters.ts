...
export type CharacterReferences = {
  ...
  profile: string;
  permissions: [
    { vocation: 'Established' | 'Freelancing' | boolean },
    { position: 'Employee' | 'Manager' | boolean },
    { occupation: 'Specialist' | 'Technician' | boolean },
  ];
};
export let returnProfiles = (pageName: 'components', blockName: 'main'): Array<CharacterReferences | HTMLElement> => {
  //--|🠋 Characters 🠋|--\\
  const characters: Array<CharacterReferences> = [
    {
